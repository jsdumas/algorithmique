...
public abstract class RussianMountainsInit {
	...
	private static final RussianMountainsQueue RUSSIAN_MOUNTAINS_QUEUE_OF_ONE_GROUP_OF_ONE_PERSON = new RussianMountainsQueueBuilder().with(GROUP_OF_ONE_PERSON).build();
	private static final RussianMountainsQueue RUSSIAN_MOUNTAINS_QUEUE_OF_FOUR_GROUPS_OF_THREE_ONE_ONE_TWO_PERSONS = new RussianMountainsQueueBuilder().with(GROUP_OF_THREE_PERSONS).with(GROUP_OF_ONE_PERSON).with(GROUP_OF_ONE_PERSON).with(GROUP_OF_TWO_PERSONS).build();
	private static final RussianMountainsQueue RUSSIAN_MOUNTAINS_QUEUE_OF_FOUR_GROUPS_OF_TWO_THREE_FIVE_THREE_PERSONS = new RussianMountainsQueueBuilder().with(GROUP_OF_TWO_PERSONS).with(GROUP_OF_THREE_PERSONS).with(GROUP_OF_FIVE_PERSONS).with(GROUP_OF_THREE_PERSONS).build();

	private static final RussianMountains RUSSIAN_MOUNTAINS_FOR_ONE_GROUP_OF_ONE_PERSON = new RussianMountains(
			MAX_PLACE_NUMBER_IS_ONE, RIDE_NUMBER_BY_DAY_IS_ONE, RUSSIAN_MOUNTAINS_QUEUE_OF_ONE_GROUP_OF_ONE_PERSON);

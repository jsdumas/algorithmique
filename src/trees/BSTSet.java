...
class BST {
	...
	static int floorrec(BST b, int x) {
		...
		if (x < b.value)
			return floor(b.left, x);
		if (x == b.value)
			return x;
		try {
			return floor(b.right, x);
	...
	/*
	 * same fringe: do two BST contain the same values? the left branch of the tree is built as a list of pairs (value, right sub-tree), of
	 * type Enum the list is bottom-up: the head is the leftmost, innermost element of the tree
	 */
	private static class Enum {
		...
		static Enum build(BST t, Enum acc) {
			if (t == null)
				return acc;
			return new Enum(t.value, t.right, build(t.left, acc));
		}
		...
		static boolean equals(Enum x, Enum y) {
			...
			if (x == null || y == null)
				return false;
			return x.root == y.root && equals(build(x.right, x.next), build(y.right, y.next));
		}
	}
	...
	static boolean subset(BST s1, BST s2) {
		...
		if (s1.value < s2.value)
			return subset(new BST(s1.left, s1.value, null), s2.left) && subset(s1.right, s2);
		else
			return subset(new BST(null, s1.value, s1.right), s2.right) && subset(s1.left, s2);
	}

	/*
	 * split(v,s) returns two trees, containing values from s smaller and greater than s
	 */
	static BST[] split(int x, BST s) {
		if (s == null)
			return new BST[] { null, null };
		if (x == s.value)
			return new BST[] { s.left, s.right };
		if (x < s.value) {
	...
	/*
	 * merge(s1, s2) merges two trees, elements of s1 being smaller than elements of s2
	 */
